...
def clean_non_json_compliant_values(data):
    if isinstance(data, dict):
        return {k: clean_non_json_compliant_values(v) for k, v in data.items()}
    if isinstance(data, list):
        return [clean_non_json_compliant_values(elem) for elem in data]
...
async def value_inference(request: Request):
    ...
    release_worker_semaphore()
    cleaned_output = clean_non_json_compliant_values(output)
