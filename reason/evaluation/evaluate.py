...
from reason.inference.lm_call import LMCallingConfig, VLLMRemoteCaller,FastChatRemoteCaller
...
if __name__ == "__main__":
    ...
    elif "qwen2.5-math-7b-prm800k" in config.RM.lower(): 
    ...
    if "qwen" or "llama" in config.LM.lower():
        lm_step_tag = "\n\n"
    else:
        lm_step_tag = "ки\n"
    ...
    def parallel_evaluate_test_dataset(
        ...
        for res_tuple in tqdm(res_q, total=len(test_ds)):
      
            if res_tuple is None:
                continue 

            problem_inst, result, output = res_tuple
